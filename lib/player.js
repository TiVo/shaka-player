...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  startBufferManagement_(mediaElement, srcEquals) {
    ...
    this.loadEventManager_.listen(mediaElement, Event.SEEKED,
        (e) => {
          // Suppress buffer-state polling during seek-based trick play or
          // scrub mode to avoid loading-indicator flicker.
          if (!this.isSeekBasedTrickPlayActive() &&
              !(this.seekBasedTrickPlayController_ &&
                this.seekBasedTrickPlayController_.isScrubActive())) {
            this.pollBufferState_(Event.SEEKED);
          }
        });
    this.loadEventManager_.listen(mediaElement, Event.SEEKED, () => {
      if (this.seekBasedTrickPlayController_) {
  ...
  startScrub() {
    ...
    // Detach the audio SourceBuffer so HTMLMediaElement.buffered reflects
    // only video, matching the behavior of seekBasedTrickPlay().
    if (this.mediaSourceEngine_ &&
        !this.config_.streaming.seekBasedTrickPlay
            .downloadAudioDuringTrickPlay) {
      this.mediaSourceEngine_.detachSourceBufferForTrickPlay(
  ...
  /**
   * End scrub mode and resume normal playback.
   *
   * @export
   */
  endScrub() {
    ...
    // Reattach the audio SourceBuffer if it was detached during startScrub.
    if (this.mediaSourceEngine_ &&
        !this.config_.streaming.seekBasedTrickPlay
            .downloadAudioDuringTrickPlay) {
      const ContentType = shaka.util.ManifestParserUtils.ContentType;
  ...
  onSeek_() {
    ...
    if (this.bufferObserver_) {
      // During seek-based trick play or scrub mode, every programmatic seek
      // fires a 'seeking' event.  Polling the buffer state here would toggle
      // STARVING↔SATISFIED 3-10×/sec, causing visible loading-indicator
      // flicker.  Skip the poll; the controller manages rendering.
      if (!this.isSeekBasedTrickPlayActive() &&
          !(this.seekBasedTrickPlayController_ &&
            this.seekBasedTrickPlayController_.isScrubActive())) {
  ...
  updateAbrManagerVariants_() {
    ...
    let variants = Array.from(adaptationSet.values());
    const currentVariant = this.streamingEngine_.getCurrentVariant();
    if (currentVariant) {
      variants = variants.filter((variant) => {
        return variant.audio.id === currentVariant.audio.id;
      });
    }
    const ret = this.abrManager_.setVariants(variants, isLowLatency);
