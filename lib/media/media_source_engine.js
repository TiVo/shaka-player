...
shaka.media.MediaSourceEngine = class {
  ...
  constructor(video, textDisplayer, playerInterface, config, lcevcDec) {
    ...
    /**
     * Content types that were successfully removed from the MediaSource by
     * detachSourceBufferForTrickPlay and are awaiting reattachment.
     *
     * @private {!Set<shaka.util.ManifestParserUtils.ContentType>}
     */
    this.detachedForTrickPlay_ = new Set();
  ...
  detachSourceBufferForTrickPlay(contentType) {
    ...
    let removedFromMediaSource = false;
    try {
      this.mediaSource_.removeSourceBuffer(sb);
      removedFromMediaSource = true;
    } catch (e) {
      shaka.log.debug('detachSourceBufferForTrickPlay: exception', e);
    }

    // Only remove from tracking if the MediaSource removal succeeded.
    // If removeSourceBuffer threw, the SB is still live in the MediaSource;
    // keep it in sourceBuffers_ so reattachSourceBufferForTrickPlay does not
    // try to addSourceBuffer a duplicate and hit QuotaExceededError.
    if (!removedFromMediaSource) {
      // Re-wire the event listeners we removed above so the buffer stays
      // functional for the remainder of the session.
      this.eventManager_.listen(sb, 'error', () => this.onError_(contentType));
      this.eventManager_.listen(
          sb, 'updateend', () => this.onUpdateEnd_(contentType));
      return;
    }
    ...
    this.detachedForTrickPlay_.add(contentType);
  ...
  reattachSourceBufferForTrickPlay(contentType) {
    ...
    // Only reattach if a prior detach actually removed the SB from the
    // MediaSource.  If the detach failed silently the SB is still live and
    // calling addSourceBuffer would exceed the MediaSource SB limit.
    if (!this.detachedForTrickPlay_.has(contentType)) {
      return;
    }

    const mimeType = this.sourceBufferTypes_.get(contentType);
    if (!mimeType) {
      this.detachedForTrickPlay_.delete(contentType);
