...
shaka.media.StreamingEngine = class {
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      // (cleared by seeked()), the guard above already excludes the
      // bufferEnd fallback — that value would come from the old buffer and
      // point to the wrong position (e.g. a forward position when we're
      // rewinding). We only reach here with a stale bufferEnd when trick
      // play is inactive.
      const reverse = this.playerInterface_.getPlaybackRate() < 0;
      let time;
      if (reverse) {
