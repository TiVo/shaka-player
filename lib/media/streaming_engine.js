...
shaka.media.StreamingEngine = class {
   ...
   * If trick play is on, related trick play streams will be used when
   * possible.
   * @param {boolean} on
   * @param {boolean=} clearBuffer When true, clears buffered normal-video
   *   data before switching to the trick-play/I-frame stream.
   */
  ...
  update_(mediaState) {
    ...
    // During seek-based trick play this check is skipped entirely:
    // bufferedAheadOf() measures FORWARD from video.currentTime, which
    // moves backward during rewind — so it always sees a large value and
    // would halt new fetches before the needed backward segments are
    // downloaded.  Normal playback uses scaledBufferingGoal.
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      // (e.g. a forward position when we're rewinding).  The outer condition
      // above already guarantees that if lastSegmentReference is null here,
      // trick play is not active, so falling back to bufferEnd is safe.
      const reverse = this.playerInterface_.getPlaybackRate() < 0;
      let time;
      if (reverse) {
