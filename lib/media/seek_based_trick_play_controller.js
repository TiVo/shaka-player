 ...
 *   <li>Render-pending gate with force-seek timeout: if a previous seek is
 *       still pending after 8× the target interval, a new seek is forced
 *       anyway to prevent stalling on slow networks.</li>
...
shaka.media.SeekBasedTrickPlayController = class {
  ...
  constructor(harness) {
     ...
     * the last time we seeked to a buffered position).  Tracked for
     * overshoot correction and future throttling enhancements.
     * @private {number}
     */
  ...
  changeRate(rate) {
    ...
    this.anchorPosition_();
    // Reset render-pending on rate change.
    this.renderPending_ = false;
   ...
   * rate and any configured seek cadence.
   *
   * By default, the interval is {@code 1 / targetFPS}.  If a positive
   * {@code seekCadence} is configured via the harness, it acts as a minimum
   * interval so the user can intentionally slow the cadence.
   *
   * @return {number} Interval in seconds.
   * @private
   */
  computeTickInterval_() {
    const adaptiveInterval = 1 / this.computeTargetFrameRate_();
    if (this.harness_ && this.harness_.getSeekCadence) {
      const cadence = this.harness_.getSeekCadence();
      if (typeof cadence == 'number' && isFinite(cadence) && cadence > 0) {
        return Math.max(adaptiveInterval, cadence);
      }
    }
