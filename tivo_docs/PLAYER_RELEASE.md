...
- Git access to the relevant `tivo/release/vX.Y.x` branch
- Push permission for tags
- `gh` CLI (optional, for checking workflow status)

## Creating a Release

### 1. Ensure you are on the target `tivo/release/vX.Y.x` branch with latest changes

```bash
git checkout tivo/release/vX.Y.x
git pull origin tivo/release/vX.Y.x
```
...
```bash
# Via gh CLI
gh run list --workflow=tivo_player_release.yaml
...
2. GitHub Actions triggers `.github/workflows/tivo_player_release.yaml`
