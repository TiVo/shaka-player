  ...
  parse(mediaSegment) {
    ...
    new Mp4Parser()
        ...
        .fullBox('tfdt', (box) => {
          ...
          if (parsedTFDT.baseMediaDecodeTimeEx) {
            const baseTime = parsedTFDT.baseMediaDecodeTimeEx;
            baseMediaDecodeTime = Math.floor(
                (baseTime.high / timescale) * Math.pow(2, 32) +
                (baseTime.low / timescale),
            );
          } else {
            baseMediaDecodeTime = parsedTFDT.baseMediaDecodeTime / timescale;
          }
        })
        .box('mdat', (box) => {
